...
let randomNumber = Math.floor(Math.random() * 100) + 1; // Corrected range
const submit = document.querySelector('#subt');
const userInput = document.querySelector("#guessField");
const guesses = document.querySelector(".guesses");
const remainingSlots = document.querySelector(".lastResult");
...
if (playGame) {
    submit.addEventListener('click', function (e) {
        e.preventDefault();
        const guess = parseInt(userInput.value);
        if (playGame) validateGuess(guess); // Prevents guessing after the game ends
    });
}
...
function checkGuess(guess) {
    if (guess === randomNumber) {
        displayMessage(`<h2 style="color:green">Congratulations! You guessed it in ${numGuesses - 1} attempts.</h2>`);
        endGame();
    } else if (guess < randomNumber) {
        displayMessage(`Number is TOO Low!`);
    } else if (guess > randomNumber) {
...
function displayGuesses(guess) {
    ...
    remainingSlots.innerHTML = `${attempts}`;
}
